...
import { CalendarIcon, Plus, Trash2, ArrowUp, ArrowDown, Eye, Vote, Shield } from "lucide-react"
import { format } from "date-fns"
import { cn } from "@/lib/utils"
import { usePostStore, type PollTypeData } from "@/lib/post-store"

const resultVisibilityOptions = [
  { value: "AFTER_VOTE", label: "After Vote", description: "Results will be visible after the poll ends.", icon: Eye },
  {
  ...
  { value: "NEVER_VISIBLE", label: "Never Visible", description: "Results will never be visible.", icon: Shield },
]
...
export function PollCreationModal({ open, onOpenChange, initialData }: PollCreationModalProps) {
  ...
  const updateOption = (index: number, value: string) => {
    setFormData((prev) => ({
      ...prev,
      options: prev.options.map((opt, i) => (i === index ? { option: value } : opt)),
    }))
  }
          ...
          <div className="space-y-6 pb-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Add at least 2 options for your poll</p>
              </div>
                ...
                {formData.options.map((option, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      />
                      {errors[`option-${index}`] && (
                        <p className="text-sm text-red-500 mt-1">{errors[`option-${index}`]}</p>
                      )}
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {formData.lastDate ? format(formData.lastDate, "PPP") : "Select end date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      onSelect={(date) => setFormData((prev) => ({ ...prev, lastDate: date }))}
                ...
                {formData.lastDate && (
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setFormData((prev) => ({ ...prev, lastDate: undefined }))}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Choose when poll results should be visible</p>
                </div>

                <Select
                  value={formData.resultVisibility}
                  onValueChange={(value: any) => setFormData((prev) => ({ ...prev, resultVisibility: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
